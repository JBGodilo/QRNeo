...
function networkUp() {
  ...
  echo "docker exec qrneocli scripts/script.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE"
  docker exec qrneocli scripts/script.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function upgradeNetwork() {
  docker inspect -f '{{.Config.Volumes}}' orderer.qrneo-network.com | grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop qrneocli
  docker-compose $COMPOSE_FILES up -d --no-deps qrneocli

  echo "Upgrading orderer"
  docker-compose $COMPOSE_FILES stop orderer.qrneo-network.com
  docker cp -a orderer.qrneo-network.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.qrneo-network.com
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.qrneo-network.com

  for PEER in peer0.taguig.qrneo-network.com peer1.taguig.qrneo-network.com peer0.magsaysay.qrneo-network.com peer1.magsaysay.qrneo-network.com; do
  ...
  docker exec qrneocli scripts/upgrade_to_v12.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/qrneo-network --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/qrneo-network/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/taguig.qrneo-network.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml 
  cd crypto-config/peerOrganizations/magsaysay.qrneo-network.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile channeltwoorgs -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for TaguigMSP   ########"
  echo "#################################################################"
  set -x
  configtxgen -profile channeltwoorgs -outputAnchorPeersUpdate ./channel-artifacts/TaguigMSPanchors.tx -channelID $CHANNEL_NAME -asOrg TaguigMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "####    Generating anchor peer update for MagsaysayMSP   ########"
  echo "#################################################################"
  set -x
  configtxgen -profile channeltwoorgs -outputAnchorPeersUpdate \
    ./channel-artifacts/MagsaysayMSPanchors.tx -channelID $CHANNEL_NAME -asOrg MagsaysayMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
