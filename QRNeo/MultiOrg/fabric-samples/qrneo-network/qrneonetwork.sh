...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/TaguigMSPanchors.tx -channelID $CHANNEL_NAME -asOrg TaguigMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for TaguigMSP..."
    exit 1
  fi
