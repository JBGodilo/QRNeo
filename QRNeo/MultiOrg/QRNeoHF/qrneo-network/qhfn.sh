...
export ORDERER_HOSTNAME="QRPDEVMACHINE3"
export ORG1_HOSTNAME="QRPUBUNTUVM1"
export ORG2_HOSTNAME="QRPUBUNTUVM2"
export SWARM_NETWORK="qrneo"
export DOCKER_STACK="qrneo"
...
function printHelp() {
  ...
  echo
  echo "	qrnetwork.sh generate -c mychannel"
  echo "	qrnetwork.sh up -c mychannel -s couchdb"
  echo "        bymn.sh up -c mychannel -s couchdb -i 1.2.x"
  echo "	qrnetwork.sh up -l node"
  echo "	qrnetwork.sh down -c mychannel"
  echo "        qrnetwork.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	qrnetwork.sh generate"
  echo "	qrnetwork.sh up"
  echo "	qrnetwork.sh down"
}
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/HF-QRNeo --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/qrneo-network/ledgers-backup
